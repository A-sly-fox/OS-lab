...
pid_t do_spawn(task_info_t *info, void* arg, spawn_mode_t mode){
    ...
    init_pcb_stack(pcb[pcb_num].kernel_sp, pcb[pcb_num].user_sp, (ptr_t)info->entry_point, &pcb[pcb_num], NULL, arg, 0);
...
pid_t do_exec(const char *file_name, int argc, char* argv[], spawn_mode_t mode){
    ...
    uint64_t kva_stack = alloc_page_helper(pcb[pcb_num].user_stack_base - PAGE_SIZE, pcb[pcb_num].pgdir);
    ...
    init_pcb_stack(pcb[pcb_num].kernel_sp, pcb[pcb_num].user_sp, entry_point, &pcb[pcb_num], (void *)uva_argv, argc, 0);
    list_add_tail(&pcb[pcb_num].list,&ready_queue);
    init_list_head(&pcb[pcb_num].wait_list);   
...
int do_mthread_create(mthread_t *thread, void (*start_routine)(void*), void *arg){
    ...
    if(process_id < NUM_MAX_TASK){
        pcb_num = process_id;
        pcb[pcb_num].kernel_stack_base = allocPage(1) + PAGE_SIZE;
        pcb[pcb_num].user_stack_base = USER_STACK_ADDR - PAGE_SIZE;
    }else{
        for (i = 0; i < NUM_MAX_TASK; i++){
            if(pcb[i].status == TASK_EXITED || pcb[i].status == TASK_ZOMBIE){
                pcb_num = i;
    ...
    pcb[pcb_num].pgdir = current_running[cpu_id]->pgdir;
    ptr_t entry_point = start_routine;
    pcb[pcb_num].kernel_sp = pcb[pcb_num].kernel_stack_base;
	pcb[pcb_num].user_sp = pcb[pcb_num].user_stack_base;
    alloc_page_helper(pcb[pcb_num].user_sp - PAGE_SIZE, pcb[pcb_num].pgdir);
	pcb[pcb_num].type = USER_THREAD;

    init_pcb_stack(pcb[pcb_num].kernel_sp, pcb[pcb_num].user_sp, entry_point, &pcb[pcb_num], arg, 1, 1);
    ...
    pcb[pcb_num].cursor_y = 1;
    pcb[pcb_num].kernel_sp = pcb[pcb_num].kernel_sp - sizeof(regs_context_t)-sizeof(switchto_context_t);
    pcb[pcb_num].mode = AUTO_CLEANUP_ON_EXIT;
    pcb[pcb_num].mask = current_running[cpu_id]->mask;
    valid_pcb++;
...
int do_mthread_join(mthread_t thread){
	return 0;
};