...
#define TRANS_OFFSET 0xffffffc000000000
...
{
    // TODO:
    return kva - (uintptr_t)TRANS_OFFSET;
}

static inline uintptr_t pa2kva(uintptr_t pa)
{
    // TODO:
    return pa + (uintptr_t)TRANS_OFFSET;
}

static inline uint64_t get_pa(PTE entry)
{
    // TODO:
    uint64_t mask = ~((~0) << RESERVED << SATP_ASID_SHIFT);
    uint64_t ppn = (entry & mask) >> RESERVED;
    return ppn << NORMAL_PAGE_SHIFT;
}
...
{
    uint64_t mask = (~0) >> RESERVED << RESERVED;
...
static inline void set_pfn(PTE *entry, uint64_t pfn)
{
    // TODO:
    uint64_t ppn = pfn << 10;
    uint64_t mask = (~0) << RESERVED >> 20 << RESERVED;
    *entry = *entry & ~mask | ppn & mask;
}
